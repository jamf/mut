...
public class tokenManagement: NSObject {
    ...
    // This function can be used to generate a token. Pass in a URL, username, and password.
    // The credentials are inserted into the header.
    public func getToken(url: String, user: String, password: String, base64Credentials: String, allowUntrusted: Bool) -> Data {
        
        var base64Creds = base64Credentials
        
        let dataPrep = dataPreparation()
        
        // If we didn't pass in credentials, concatenate the username and password and encode.
        if base64Creds == "" {
            // Call the data manipulation class to base64 encode the credentials
            base64Creds = dataPrep.base64Credentials(user: user, password: password)
